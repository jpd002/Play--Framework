...
{
	InitCommonControls();
	Create(nExStyle, WC_HEADER, _X(""), WS_CHILD | WS_VISIBLE | nStyle, pRect, hParent, NULL);
}

int CHeader::InsertItem(const xchar* sText)
{
	HDITEM Item;

	memset(&Item, 0, sizeof(HDITEM));
	Item.mask		= HDI_TEXT;
	Item.pszText	= const_cast<xchar*>(sText);

	return InsertItem(GetItemCount(), &Item);
}

int CHeader::InsertItem(const xchar* sText, unsigned int nSize)
{
	HDITEM Item;
